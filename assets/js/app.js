...
function normalizeData(data){
  ...
  if(data.maintenanceDecision!=='contract'){
    data.subscription=false;
    data.printMode='implementation';
  }
  const legacyAutoNames=new Set(['','新規導入案件','名称未設定',...CASE_TEMPLATES.map(template=>`${template.label} 案件`)]);
  if(!['auto','manual'].includes(data.caseNameMode)){
    data.caseNameMode=legacyAutoNames.has(String(data.caseName||'').trim())||/のみ 案件$/.test(String(data.caseName||'').trim())?'auto':'manual';
  }
...
function markDirty(){state.dirty=true;$('#saveState').textContent='未保存の変更';if($('#topSaveState'))$('#topSaveState').textContent='未保存';localStorage.setItem(UI_KEY,JSON.stringify(state.data))}
...
function applyCaseTemplate(id){
  ...
  state.data=next;
  normalizeData(state.data);
  refreshAutoCaseName(state.data);
  markDirty();
...
function syncServiceBase(serviceId,enabled){
  ...
  if(!enabled)state.data.quotes=state.data.quotes.filter(q=>!(q.source==='hearingOption'&&q.category===base.category));
  refreshAutoCaseName(state.data);
  markDirty();
}
...
function customerQuotePrice(q){
  if(Number(q.price)===0)return {visible:false,label:'別途見積'};
  return {visible:true,label:yen((Number(q.qty)||0)*(Number(q.price)||0))};
}
function customerEstimateSummaryHtml(){const rows=implementationQuoteLines(state.data),totals=calculateQuoteTotals(rows),hasIndividual=rows.some(q=>!customerQuotePrice(q).visible);return `<section class="card hearing-estimate"><div class="hearing-estimate-head"><div><span class="eyebrow">ESTIMATE PREVIEW</span><h2>現在の概算見積</h2><p>基本サービスと、見積へ追加したオプションを自動集計しています。</p></div><b>${yen(totals.total)}<small> 税込</small></b></div><div class="hearing-estimate-rows">${rows.length?rows.map(q=>{const shown=customerQuotePrice(q);return `<div><span><b>${esc(q.name)}</b><small>${esc(q.category||'その他')} / ${q.qty}${esc(q.unit||'式')}</small></span><strong class="${shown.visible?'':'individual-price'}">${shown.label}</strong></div>`}).join(''):'<p class="muted">サービスまたはオプションを選択してください。</p>'}</div><div class="hearing-estimate-total"><span>税別合計 ${yen(totals.subtotal)} ＋ 消費税 ${yen(totals.tax)}</span><button class="button ghost" data-go="pricing">見積書で確認</button></div>${hasIndividual?'<p class="individual-estimate-note">「別途見積」の項目は、上記合計に含まれていません。</p>':''}<p class="estimate-caution">この金額はヒアリング時点の概算です。仕様・素材・機器・訪問範囲を確認後、正式な見積書を発行します。</p></section>`}
...
function estimateCaseTitle(targets){
  const selected=SERVICE_ORDER.filter(id=>targets.includes(id));
...
function totalsHtml(data=state.data,lines=implementationQuoteLines(data)){const totals=calculateQuoteTotals(lines);return `<div class="total-line"><span>小計</span><b>${yen(totals.subtotal)}</b></div><div class="total-line"><span>消費税（10%）</span><b>${yen(totals.tax)}</b></div><div class="total-line grand"><span>合計</span><b>${yen(totals.total)}</b></div>`}
...
function quoteIssuer(d,caseTitle=d.caseName){return `<div class="quote-party-row"><div><b>${esc(d.storeName||'　　　　　　　　　')} 御中</b><span>案件名：${esc(caseTitle)}</span></div><div class="quote-issuer ${d.includeStamp?'has-stamp':''}"><span>発行者</span><b>PCSAPO マキシ企画</b><small>発行日：${esc(d.date)}<br>契約番号：${esc(d.contractNo)}</small>${d.includeStamp?'<img class="quote-stamp" src="assets/images/pcsapo-stamp.png" alt="PCSAPO マキシ企画 印">':''}</div></div>`}
function quotePrintSheet(d){const rows=implementationQuoteLines(d);return `<section class="card print-sheet"><div class="print-doc-head"><span>02</span><div><small>QUOTATION</small><h2>御見積書</h2></div></div>${quoteIssuer(d)}<div class="table-wrap"><table class="data-table"><thead><tr><th>区分</th><th>項目</th><th>数量</th><th>単価</th><th>小計</th></tr></thead><tbody>${rows.map(q=>`<tr class="${q.source==='option'||q.source==='hearingOption'?'print-option-row':''}"><td>${esc(q.category||inferQuoteCategory(q.name,q.source))}</td><td>${q.source==='option'||q.source==='hearingOption'?'<small>OPTION</small> ':''}${esc(q.name)}</td><td>${q.qty}${q.unit?` ${esc(q.unit)}`:''}</td><td>${yen(q.price)}</td><td>${yen(q.qty*q.price)}</td></tr>`).join('')}</tbody></table></div><div class="quote-totals">${totalsHtml(d,rows)}</div><section class="card notice"><b>備考：</b>外部サービス利用料、機器代、印刷代等は別途となる場合があります。</section></section>`}
...
function currentPageLabel(){
  const menu=ADMIN_MENUS.flatMap(([,items])=>items).find(([id])=>id===state.page);
  return menu?.[2]||'管理者ページ';
}
function render(){
  ...
  $('.topbar').hidden=entry;
  $('#caseWorkspace').hidden=state.mode!=='admin';
  ...
  if(state.mode==='admin'){
    ...
    $('#topSaveState').textContent=state.dirty?'未保存':'保存済み';
    $('#autoCaseName').classList.toggle('active',state.data.caseNameMode==='auto');
    $('#autoCaseName').textContent=state.data.caseNameMode==='auto'?'自動中':'自動名';
  }
...
function bindPage(){
  $$('[data-field]').forEach(el=>{const update=()=>{const k=el.dataset.field;state.data[k]=el.type==='checkbox'?el.checked:el.value;if(k==='caseName'){$('#caseName').value=el.value;state.data.caseNameMode='manual'}if(k==='maintenanceDecision'){state.data.subscription=el.value==='contract';if(el.value!=='contract')state.data.printMode='implementation';refreshAutoCaseName(state.data);markDirty();render();return}if(k==='plan'){state.data.subscriptionPrice=MAINTENANCE_PLANS[el.value]?.price||state.data.subscriptionPrice;markDirty();render();return}markDirty()};el.addEventListener('input',update);el.addEventListener('change',update)});
  ...
  $$('[data-set-print-mode]').forEach(el=>el.onclick=()=>{const requested=el.dataset.setPrintMode;if(requested!=='implementation'&&!hasActiveMaintenance(state.data)){state.data.printMode='implementation';toast('保守契約と保守見積を確認してから選択してください');render();return}state.data.printMode=requested;markDirty();navigate('print')});
...
function quoteTotalFor(x){return calculateQuoteTotals(implementationQuoteLines(x)).total}
